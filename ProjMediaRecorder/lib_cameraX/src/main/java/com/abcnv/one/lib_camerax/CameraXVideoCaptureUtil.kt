...
object CameraXVideoCaptureUtil {
...
//        previewView.controller = cameraController
    ...
    fun bindPreviewOfVideoCapture(
        ...
        try {
            ...
            val qualitySelector = QualitySelector.fromOrderedList(
                ...
                FallbackStrategy.lowerQualityOrHigherThan(Quality.SD)//质量较低或高于
            )

            val recorder = Recorder.Builder()
                .setExecutor(cameraExecutor!!)
                .setQualitySelector(qualitySelector)
                .setAspectRatio(screenAspectRatio).build()
    ...
    private fun Quality.quality2Str(): String {
        return when (this) {
            Quality.UHD -> "UHD"
            Quality.FHD -> "FHD"
            Quality.HD -> "HD"
    ...
    fun startRecording(context: Context, path: String) {
        ...
        val contentValues = ContentValues().apply {
            put(MediaStore.Video.Media.DISPLAY_NAME, saveFile!!.absolutePath)
        }
        val uri = MediaStore.Video.Media.EXTERNAL_CONTENT_URI
        val mediaStoreOutput = MediaStoreOutputOptions.Builder(context.contentResolver, uri)
            .setContentValues(contentValues)
            .build()

        videoCapture?.let {
            recording = it.output.prepareRecording(context, mediaStoreOutput)
                .withAudioEnabled()
                .start(
                    ContextCompat.getMainExecutor(context.applicationContext),
                    object : Consumer<VideoRecordEvent> {
                        override fun accept(t: VideoRecordEvent) {
                            println("文件大小：${t.outputOptions.fileSizeLimit},时长:${t.outputOptions.durationLimitMillis},byte size:${t.recordingStats.numBytesRecorded},")
                        }
                    })
        }
    }
    ...
    fun stopRecord(context: Context) {
        ...
        Toast.makeText(context, "保存成功: ${saveFile?.absolutePath}", Toast.LENGTH_SHORT).show()
    }
